<x-app-layout>
    ...
    <x-slot:title>
        @if(auth()->user()->role->name == 'manager')
            Received Applications - Depozit.uz
        @else
            Create Application - Depozit.uz
        @endif
    </x-slot:title>
    <x-slot name="header">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    @if(auth()->user()->role->name == 'manager')
                        <span class="font-bold text-xl text-blue-500">Received Applications</span>
                                    ...
                                    <div class="rounded-xl border p-5 mt-5 shadow-md w-9/12 bg-white">
                                        <div class="flex w-full items-center justify-between border-b pb-3">
                                            <div class="flex items-center space-x-3">
                                                <div class="h-10 w-10 rounded-full bg-slate-400">
                                                    <img class="rounded-full"
                                                         src="{{asset('storage/'.$application->user->photo)}}" alt="">
                                                </div>
                                                <div
                                        ...
                                        <div class="flex justify-between">
                                            <div>
                                                <div class="mt-4 mb-3">
                                                    <div class="mb-3 text-xl font-bold">{{$application->subject}}
                                                    </div>
                                        ...
                                        <div class="flex justify-end">
                                            <a href="{{route('answers.create', ['application' => $application->id])}}"
                                               class="bg-green-500 text-white px-3 py-1 rounded font-small mx-3 hover:bg-green-600 transition duration-200 each-in-out">Reply</a>
                                        </div>
                                    </div>
                                @endif
                            @endforeach
                ...
                @else
                    ...
                    @elseif(session()->has('create'))
                        <div class="flex bg-green-100 rounded-lg p-4 mb-4 text-sm text-green-700" role="alert">
                            ...
                            <div>
                                <span class="font-medium">Success! </span> {{session()->get('create')}}
                            </div>
                        </div>
                    @endif
                        ...
                        <div class='w-full max-w-lg px-10 py-8 mx-auto bg-white rounded-lg shadow-xl'>
                            <div class='max-w-md mx-auto space-y-6'>
                                ...
                                <form action="{{route('applications.store')}}" method="POST"
                                    ...
                                    <textarea rows="5" type="text" required name="message"
